...
@section('content')
    <div class="container px-6 mx-auto grid">
        ...
        <form action="{{ route('admin.ads-manager.update', $adsManager->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="border rounded-xl p-4 sm:p-6 lg:p-8 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-xl space-y-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ $adsManager->name }}" class="py-3 px-4 block w-full border-gray-200 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400" required>
                    @error('name')
                        <p class="text-xs text-red-600 mt-2">{{ $message }}</p>
                    @enderror
                </div>
                <div x-data="{
                    adType: '{{ $adsManager->adType }}'
                }">
                    ...
                    <div x-show="adType == 'HTML'">
                        <div>
                            ...
                            <textarea name="body" id="body" class="py-3 px-4 block w-full border-gray-200 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400" cols="30" rows="10">{{  $adsManager->body }}</textarea>
                            @error('body')
                                <p class="text-xs text-red-600 mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div x-show="adType == 'IMAGE'">
                        <div>
                            ...
                            <div>
                                ...
                                <input type="url" name="imageUrl" placeholder="{{ __('main.image_url') }}" value="{{ $adsManager->imageUrl }}" id="imageUrl" class="block w-full border border-gray-200 shadow-sm rounded-md text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 file:bg-transparent file:border-0 file:bg-gray-100 file:mr-4 file:py-3 file:px-4 dark:file:bg-gray-700 dark:file:text-gray-400">
                            ...
                            <div>
                                <label for="imageAlt"
                                    class="block text-sm mb-2 text-gray-700 font-medium dark:text-white">{{ __('main.image_alt') }}</label>
                                <input type="text" name="imageAlt" placeholder="{{ __('main.image_alt') }}" value="{{ $adsManager->imageAlt }}"
                                    id="imageAlt"
                ...
                <div class="my-3">
                    <button type="submit" class="py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800">{{ __('main.update') }}</button>
                </div>
